...
export function HomeTab({ links, categories }: Props) {
  ...
  // Recent links: pinned first, then newest-to-oldest
  ...
  const getBreadcrumb = (link: Link) => {
    ...
    const section = link.type === 'Web' ? 'Web' : link.type === 'Document' ? 'Docs' : link.subtype;
    ...
    <div className="space-y-7">
      ...
      <div className="animate-slide-up" style={{ animationDelay: '0.1s' }}>
        ...
        {recentLinks.length === 0 ? (
          ...
          <DraggableLinkList links={recentLinks} categories={categories} getBreadcrumb={getBreadcrumb} />
        )}
      </div>
    </div>
  );
