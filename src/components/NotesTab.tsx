...
function ToolButton({ label, extra, onClick }: { label: string; extra: string; onClick: () => void }) {
  ...
  return (
    <button
      ...
      className={`px-2.5 py-1.5 rounded-[10px] text-xs font-medium text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-primary)] transition active:scale-95 select-none ${extra}`}
    >
      {label}
    </button>
  );
