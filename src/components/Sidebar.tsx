...
import {
  CloseIcon, SunIcon, MoonIcon, SyncIcon, GoogleIcon,
  MailIcon, InfoIcon, LogOutIcon, BookmarkIcon
} from './Icons';
...
export function Sidebar({ open, onClose, theme, toggleTheme, user, onSignInGoogle, onSignInEmail, onSignOut }: Props) {
  ...
  const handleSync = async () => {
    ...
    setSyncing(false);
    setSyncStatus(result.success ? 'Synced successfully' : `Error: ${result.error}`);
    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-[300px] z-50 transform transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${open ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        <div className="flex flex-col h-full border-r border-[var(--surface-border)]">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
            ...
            <div>
              ...
              {user ? (
                <div className="space-y-2.5">
                  <div className="px-4 py-3.5 card-sm" style={{ borderColor: 'rgba(108,92,231,0.15)' }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-semibold text-[var(--accent)] uppercase tracking-wider">Connected</p>
                        <p className="text-sm text-[var(--text-primary)] truncate mt-0.5">{user.email}</p>
                      </div>
                    </div>
                  </div>
                  <button onClick={handleSync} disabled={syncing} className="w-full flex items-center justify-center gap-2 px-4 py-3.5 card-sm text-sm font-medium text-[var(--accent)] hover:bg-[var(--surface-hover)] disabled:opacity-50 transition">
                    <SyncIcon size={15} className={syncing ? 'animate-spin' : ''} />
                    {syncing ? 'Syncing…' : 'Sync Data'}
                  </button>
                  {syncStatus && (
                    <p className={`text-[11px] px-3 font-medium ${syncStatus.includes('Error') ? 'text-red-500' : 'text-green-500'}`}>
                      {syncStatus}
                    </p>
                  )}
                  <button onClick={onSignOut} className="w-full flex items-center justify-center gap-2 px-4 py-3.5 card-sm text-sm font-medium text-red-500 hover:bg-red-500/5 transition">
                    <LogOutIcon size={15} />
                    Log Out
                  </button>
                </div>
                ...
                <div className="space-y-2.5">
                  <button onClick={onSignInGoogle} className="w-full flex items-center justify-center gap-3 px-4 py-3.5 card-sm text-sm font-medium text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition">
                    <GoogleIcon size={17} />
                    Continue with Google
                  </button>
                  <button onClick={() => setShowAuth(!showAuth)} className="w-full flex items-center justify-center gap-2.5 px-4 py-3.5 card-sm text-sm font-medium text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition">
                    <MailIcon size={15} />
                    Continue with Email
                  </button>
                  {showAuth && (
                    <div className="space-y-2.5 p-4 card-sm animate-scale-in">
                      <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" className="input-field text-sm" />
                      <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" className="input-field text-sm"
                        onKeyDown={e => e.key === 'Enter' && handleEmailAuth()} />
                      {authError && <p className="text-[11px] text-red-500 font-medium">{authError}</p>}
                      <button onClick={handleEmailAuth} className="btn-primary w-full text-sm py-3">
                        {isSignUp ? 'Create Account' : 'Sign In'}
                      </button>
                      <button onClick={() => setIsSignUp(!isSignUp)} className="text-[11px] text-[var(--accent)] font-medium hover:opacity-80 transition w-full text-center">
            ...
            {/* About */}
            <div>
              <button onClick={() => setShowAbout(true)} className="w-full flex items-center gap-3 px-4 py-3.5 card-sm text-sm font-medium text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition">
                <InfoIcon size={16} />
                About Bookmarkly
              </button>
            </div>
          ...
          <div className="px-6 py-4 border-t border-[var(--surface-border)]">
            <p className="text-[10px] text-[var(--text-tertiary)] text-center">Bookmarkly v1.0 · Your data, your control</p>
          </div>
        </div>
      </div>
      ...
      {showAbout && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-5">
          ...
          <div className="relative w-full max-w-sm animate-scale-in" style={{ background: theme === 'dark' ? 'rgba(15,15,22,0.97)' : 'rgba(255,255,255,0.97)', backdropFilter: 'blur(24px)', borderRadius: '24px', border: '1px solid var(--surface-border)', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
