...
export function LinkItem({ link, categories, categoryName }: Props) {
  ...
  const isDataUrl = link.url.startsWith('data:');
  const displayText = link.name || link.file_name || (isDataUrl ? 'Uploaded file' : link.url);
  const truncated = displayText.length > 55 ? displayText.slice(0, 55) + '…' : displayText;

  const handleSave = () => {
    updateLink(link.id, { name, url, category_id: catId || null, notes });
    setEditing(false);
  };
