...
export async function updateCategory(id: string, updates: Partial<Pick<Category, 'name' | 'color' | 'position'>>) {
  const all = loadLocal<Category>(CATEGORIES_KEY);
...
export async function moveCategory(id: string, direction: 'left' | 'right', type: LinkType, subtype: LinkSubtype) {
  const all = loadLocal<Category>(CATEGORIES_KEY);
  const visible = all.filter(c => !c.is_deleted && c.type === type && c.subtype === subtype)
  ...
  const tempPos = visible[idx].position;
  visible[idx].position = visible[swapIdx].position;
  visible[swapIdx].position = tempPos;
  visible[idx].updated_at = new Date().toISOString();
  visible[swapIdx].updated_at = new Date().toISOString();

  // Write back
  for (const v of [visible[idx], visible[swapIdx]]) {
    const ai = all.findIndex(c => c.id === v.id);
    if (ai >= 0) { all[ai] = v; }
  }
  saveLocal(CATEGORIES_KEY, all);
  notify();
  const userId = getUserId();
  if (userId) {
    await upsertCategory(visible[idx], userId);
    await upsertCategory(visible[swapIdx], userId);
  }
}
...
export async function addLink(
  ...
  const now = new Date().toISOString();
  const userId = getUserId();
  const existing = getLinks().filter(l => l.type === type && l.subtype === subtype);
  const maxPos = existing.length > 0 ? Math.max(...existing.map(l => l.position ?? 0)) : -1;

  const link: Link = {
    id: uuidv4(), url, name: name || '', type, subtype,
    category_id: categoryId, position: maxPos + 1,
    created_at: now, updated_at: now, is_deleted: false,
...
export async function updateLink(id: string, updates: Partial<Pick<Link, 'url' | 'name' | 'category_id' | 'notes' | 'position'>>) {
  const all = loadLocal<Link>(LINKS_KEY);
...
export async function moveLink(id: string, direction: 'up' | 'down', filteredLinks: Link[]) {
  const sorted = [...filteredLinks].sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
  ...
  const all = loadLocal<Link>(LINKS_KEY);
  const aIdx = all.findIndex(l => l.id === sorted[idx].id);
  const bIdx = all.findIndex(l => l.id === sorted[swapIdx].id);
  if (aIdx < 0 || bIdx < 0) return;

  const tempPos = all[aIdx].position;
  all[aIdx].position = all[bIdx].position;
  all[bIdx].position = tempPos;
  all[aIdx].updated_at = new Date().toISOString();
  all[bIdx].updated_at = new Date().toISOString();

  saveLocal(LINKS_KEY, all);
  notify();
  const userId = getUserId();
  if (userId) {
    await upsertLink(all[aIdx], userId);
    await upsertLink(all[bIdx], userId);
  }
}
...
export function exportToCSV(): { categories: string; links: string } {
  ...
  const catHeader = 'id,name,type,subtype,color,position,created_at,updated_at';
  const catRows = cats.map(c =>
    `"${c.id}","${esc(c.name)}","${c.type}","${c.subtype}","${c.color || ''}","${c.position}","${c.created_at}","${c.updated_at}"`
  );

  const linkHeader = 'id,url,name,type,subtype,category_id,position,notes,created_at,updated_at';
  const linkRows = lnks.map(l =>
    `"${l.id}","${esc(l.url)}","${esc(l.name)}","${l.type}","${l.subtype}","${l.category_id || ''}","${l.position}","${esc(l.notes || '')}","${l.created_at}","${l.updated_at}"`
  );
...
export async function deleteAllUserData(): Promise<{ success: boolean; message: string }> {
  ...
  try {
    ...
    // Try to call the database function to delete auth user
    try {
      await supabase.rpc('delete_user_account', { target_user_id: userId });
    } catch {
      // Function might not exist yet, data is still deleted
      console.warn('[Store] delete_user_account RPC not available — data deleted but auth record remains');
    }
