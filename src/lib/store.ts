...
function migrateKeys() {
  const oldKeys = ['mindcache_links', 'mindcache_categories', 'mindcache_notes'];
  const newKeys = [LINKS_KEY, CATEGORIES_KEY, NOTES_KEY];
  oldKeys.forEach((old, i) => {
    const data = localStorage.getItem(old);
    if (data && !localStorage.getItem(newKeys[i])) {
...
export async function addCategory(name: string, type: LinkType, subtype: LinkSubtype = 'None'): Promise<Category> {
  ...
  const cat: Category = { id: uuidv4(), name, type, subtype, created_at: now, updated_at: now, is_deleted: false };
...
async function syncCategoryToSupabase(cat: Category) {
  ...
  try {
    await supabase.from('categories').upsert({
      ...
      created_at: cat.created_at, updated_at: cat.updated_at, is_deleted: cat.is_deleted, user_id: userId,
    });
  } catch (e) { console.error('Sync category error', e); }
}
...
export async function addLink(url: string, type: LinkType, subtype: LinkSubtype, categoryId: string | null, name?: string, fileName?: string, fileData?: string): Promise<Link> {
  const now = new Date().toISOString();
  const link: Link = {
    id: uuidv4(), url, name: name || '', type, subtype, category_id: categoryId,
    created_at: now, updated_at: now, is_deleted: false,
    file_name: fileName, file_data: fileData, notes: '',
  };
...
async function syncLinkToSupabase(link: Link) {
  ...
  try {
    await supabase.from('links').upsert({
      id: link.id, url: link.url, name: link.name, type: link.type, subtype: link.subtype,
      category_id: link.category_id, created_at: link.created_at, updated_at: link.updated_at,
      is_deleted: link.is_deleted, file_name: link.file_name || null, notes: link.notes || '', user_id: userId,
    });
  } catch (e) { console.error('Sync link error', e); }
}
...
export async function addNote(title: string, body: string = ''): Promise<Note> {
  const now = new Date().toISOString();
  const note: Note = { id: uuidv4(), title, body, created_at: now, updated_at: now, is_deleted: false };
...
async function syncNoteToSupabase(note: Note) {
  ...
  try {
    await supabase.from('notes').upsert({
      ...
      created_at: note.created_at, updated_at: note.updated_at, is_deleted: note.is_deleted, user_id: userId,
    });
...
export async function pullFromSupabase(): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const { data: remoteCats, error: catErr } = await supabase.from('categories').select('*').eq('user_id', userId);
    if (catErr) throw catErr;
    const { data: remoteLinks, error: linkErr } = await supabase.from('links').select('*').eq('user_id', userId);
    if (linkErr) throw linkErr;
    const { data: remoteNotes, error: noteErr } = await supabase.from('notes').select('*').eq('user_id', userId);

    const localCats = loadLocal<Category>(CATEGORIES_KEY);
    saveLocal(CATEGORIES_KEY, mergeArrays(localCats, (remoteCats || []) as Category[]));

    const localLinks = loadLocal<Link>(LINKS_KEY);
    saveLocal(LINKS_KEY, mergeArrays(localLinks, (remoteLinks || []) as Link[]));

    if (!noteErr && remoteNotes) {
      const localNotes = loadLocal<Note>(NOTES_KEY);
      saveLocal(NOTES_KEY, mergeArrays(localNotes, remoteNotes as Note[]));
    }

    notify();

    for (const cat of loadLocal<Category>(CATEGORIES_KEY)) await syncCategoryToSupabase(cat);
    for (const link of loadLocal<Link>(LINKS_KEY)) await syncLinkToSupabase(link);
    for (const note of loadLocal<Note>(NOTES_KEY)) await syncNoteToSupabase(note);
...
function mergeArrays<T extends { id: string; updated_at: string }>(local: T[], remote: T[]): T[] {
  ...
  for (const item of remote) {
    ...
    if (!existing || new Date(item.updated_at) > new Date(existing.updated_at)) map.set(item.id, item);
  }
